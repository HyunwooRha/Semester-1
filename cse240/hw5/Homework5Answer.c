...
void addMap(char* mapNameInput, unsigned int enemyCount)
{
	...
	// Set the data of the new map
	strcpy(newMap->name, mapNameInput);
	newMap->enemyCount = enemyCount;
	newMap->hero = NULL;
	newMap->enemyList = NULL;

	// Add the new map to the head of the list
	struct mapList* newMapList = malloc(sizeof(struct mapList));
	newMapList->map = newMap;
	newMapList->next = list;
	list = newMapList;
...
{
	// Traverse the linked list to find the map with the specified name
	struct mapList* currentMap = list;
	while (currentMap != NULL) {
		if (strcmp(currentMap->map->name, mapNameInput) == 0) {
			// Map found, return a pointer to the map struct
			return currentMap->map;
		}
		currentMap = currentMap->next;
	}

	// Map not found, return NULL
	return NULL;
}
...
void createHero(char* heroNameInput, unsigned int heroHpInput, unsigned int heroAttackInput){
	...
	if (theHero != NULL) {
		// Hero already exists, remove them from the map
		struct mapList* currentMap = list;
		while (currentMap != NULL) {
			if (currentMap->map->hero == theHero) {
				currentMap->map->hero = NULL;
				break;
			}
			currentMap = currentMap->next;
		}

		// Free the memory allocated for the hero
		free(theHero);
	}

	// Allocate memory for the new hero
	theHero = malloc(sizeof(struct hero));

	// Set the data of the new hero
...
void moveHero(char* mapNameInput){
	// Find the map with the specified name
	struct map* map = searchMap(mapNameInput);
	if (map == NULL) {
		return;
	}

	// Move the hero to the specified map
	map->hero = theHero;
...
void addEnemy(char* mapNameInput, char* enemyNameInput, unsigned int enemyHpInput, unsigned int enemyAttackInput)
{
	...
	// Set the data of the new enemy
	strcpy(newEnemy->name, enemyNameInput);
	newEnemy->hp = enemyHpInput;
	newEnemy->attack = enemyAttackInput;
	newEnemy->next = NULL;

	// Add the new enemy to the tail of the list
	if (map->enemyList == NULL) {
		map->enemyList = newEnemy;
	} else {
		struct enemy* currentEnemy = map->enemyList;
...
void removeMap(char* mapNameInput)
{
	// Find the map with the specified name
	struct mapList* currentMap = list;
	struct mapList* previousMap = NULL;
	while (currentMap != NULL) {
		if (strcmp(currentMap->map->name, mapNameInput) == 0) {
			break;
		}
	...
	// Remove the enemies from the map
	struct enemy* currentEnemy = currentMap->map->enemyList;
	...
	if (previousMap == NULL) {
		list = currentMap->next;
	} else {
		previousMap->next = currentMap->next;
	}
	free(currentMap);
...
int mapCombat(char* mapNameInput)
{
	...
	// Check if the hero is in the map
	if (map->hero == NULL) {
		return 2;
	}
	...
	// Fight the enemies
	int result = combatRecurse(map->hero, map->enemyList);
...
int combatRecurse(struct hero* Hero, struct enemy* enemyList)
{
	...
	if (enemyList->hp <= 0) {
		return combatRecurse(Hero, enemyList->next);
	}
